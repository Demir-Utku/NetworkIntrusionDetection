...
nbrOfNeuronsInEachHiddenLayer = [100 100]; %linear:[4] - circle:[10] - wave,spirals:[10 10]
nbrOfOutUnits = 10;
...
draw_each_nbrOfEpochs = 101;

%% Read Data
Samples = table2array(trainData(:,1:40));
TargetClasses = table2array(trainData(:,41));
ActualClasses = -1*ones(size(TargetClasses));


%% Calculate Number of Input and Output NodesActivations
nbrOfInputNodes = 40; %=Dimention of Any Input Samples
...
for i=1:length(TargetClasses)
    x=eye(10,10);
    TargetOutputs(i,:) = x(TargetClasses(i),:);
...
% Samples = table2array(testData(:,1:40));
% TargetClasses = table2array(testData(:,41));
% 
% % Evaluation
%     results=zeros([1 10]);
...
%         outputs = EvaluateNetwork(Samples(Sample,:), NodesActivations, Weights, unipolarBipolarSelector);
%         [val, idx] = max(outputs);
%         predictions(Sample)=idx;
%         if TargetClasses(Sample)==idx
%             results(idx)=results(idx)+1;
%         end
%         class_number(TargetClasses(Sample))=class_number(TargetClasses(Sample))+1;
%     end
% 
% acc=(results./class_number)*100;
% mean(acc);
% 
% cmat=confusionmat(TargetClasses,predictions);
% plotconfusion(TargetClasses,predictions);
% 
% for i=1:size(cmat,1)
